...
# variable, and the coffee types filter.
#
...
dashboard_events_init <- function(db) {
  ...
  tryCatch({
    ...
    DBI::dbExecute(db$db, sprintf(
      ...
      dashboard_events_table))
    # For tables created before the dragon_species -> coffee_types rename.
    DBI::dbExecute(db$db, sprintf("
      DO $$
      BEGIN
        IF EXISTS (
          ...
          WHERE table_schema = 'public' AND table_name = '%s'
            AND column_name = 'dragon_species'
        ) AND NOT EXISTS (
          SELECT 1 FROM information_schema.columns
          WHERE table_schema = 'public' AND table_name = '%s'
            AND column_name = 'coffee_types'
        ) THEN
          ALTER TABLE public.%s RENAME COLUMN dragon_species TO coffee_types;
        END IF;
      END $$;", dashboard_events_table, dashboard_events_table, dashboard_events_table))
    DBI::dbExecute(db$db, sprintf(
...
#'     "bar_var", "coffee_types", or several joined with "+".
log_dashboard_event <- function(db, design, session_id, shiny_token, event,
                                scatter_x, scatter_y, bar_var, coffee_types) {
  tryCatch({
    if (is.null(db) || is.null(db$db)) {
      ...
      row <- data.frame(
        ...
        bar_var = bar_var, coffee_types = coffee_types,
