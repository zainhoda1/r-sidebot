...
#
# `session_id` is the survey session id (falling back to `session$token`); pass
# it everywhere so events from concurrent participants can be told apart.
...
log_app_message <- function(text, event = "app_message",
                            context = "explain_panel", session_id = NA) {
  .log_event(
...
log_query <- function(user_msg, context = "explain_panel", session_id = NA) {
  .log_event(
...
log_exchange <- function(user_msg, assistant_text, context = "explain_panel",
                         session_id = NA) {
...
#'
#' `plot` is the plot signature (chart type, species, axes) the image was made
#' from, so the transcript says which view the assistant was actually shown.
log_plot_image <- function(plot = "", context = "explain_panel", session_id = NA) {
  .log_event(
    list(timestamp = .now(), session_id = session_id, event = "plot_image_sent",
         context = context, plot = as.character(plot)),
    paste0("IMAGE: current plot attached to the user's message [", plot, "]\n")
  )
}
...
log_conversation <- function(chat, session_id, context = "explain_panel") {
  tryCatch({
