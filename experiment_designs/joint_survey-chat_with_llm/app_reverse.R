...
dragons_opposite <- readr::read_csv(here("data", "dragons_opposite.csv"))
...
server <- function(input, output, session) {
  ...
  dragons_opposite_data <- reactive(dragons_opposite)
  ...
  opposite_plot <- reactive({
    req(nrow(dragons_opposite_data()) > 0)
    df <- dragons_opposite_data()
    env <- new.env(parent = globalenv())
    env$df <- df
    tryCatch(
      eval(parse(text = plot_code()), envir = env),
  ...
  observeEvent(input$interpret_custom, {
    explain_plot(chat, custom_plot(), "chat", session, p_opposite = opposite_plot())
  })
