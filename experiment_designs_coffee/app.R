...
# "dragon_species" so the existing dashboard_events table schema keeps working.
# Only the labels shown to participants changed.
...
server <- function(input, output, session) {
  ...
  scatter_plot_r <- reactive({
    req(nrow(farms_data()) > 0, input$scatter_x, input$scatter_y, length(input$dragon_species) > 0)
    all_selected <- "All Varietals" %in% input$dragon_species
    df <- if (all_selected) {
      farms_data()
    } else {
      farms_data() |> filter(varietal %in% input$dragon_species)
    }
  ...
  bar_plot_r <- reactive({
    req(nrow(farms_data()) > 0, input$bar_var, length(input$dragon_species) > 0)
    df <- if ("All Varietals" %in% input$dragon_species) {
      farms_data()
    } else {
      farms_data() |> filter(varietal %in% input$dragon_species)
    }
